...
We chose to calculate the normalize log-weighted term frequencies for each term
...
term frequencies We have to store floats. To save a bit of space, instead of
writing the entire float to the postings file, We first remove the first 2
characters of the float, i.e. the '0.'. 
...
raw term frequencies and normalized log-weighted term frequecies, [(1, 2, 0.123), (2, 3, 0.456), (3, 4, 0.789)].
The string that is output that is to be written to the file is:
"1 2 123 2 3 456 3 4 789\n". Notice that the '0.' before each of the normalized log-
weighted term frequecies have been removed as explained in the previous
paragraph as a space saving measure. The normalised log-weighted tf is used for VSM, while the raw term frequency was used for the LM.
...
list, and the inverse document frequency into the dictionary file. We calculate
the idf of each document during this indexing phase for the same reason as We do
...
The output of the of the two functions, normalize_query_term_frequencies
...
Lastly, the cosine scores of the documents are multiplied by each of the IPC categories scores to give the final scores, sorted in descending order.
...
In our indexing step, we indexed both the document id and the IPC categories. How we index the IPC scores? Also got use the normalisation. Not just raw frequency.
...
As an illustration, document 1 has a VSM score of 0.3 for the query "Washing machine", IPC class score of 0.1, IPC subclass score of 0.7 and an IPC class score of 0.9. We also squared each of the IPC categories score because we have found out through experimentation that it improves the F2 score.

The final score would be 0.3 * 0.1^2 * 0.7^2 * 0.9^2 = 0.0011907
