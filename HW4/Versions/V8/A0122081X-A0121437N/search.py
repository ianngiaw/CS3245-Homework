...
pseudo_relevance_threshold = 0.07
...
def execute_query(input_post_file, input_query_file, output_file, term_dict, docs_dict, doc_fields_dict):
    ...
    lm_threshold = int(len(LM_results) * pseudo_relevance_threshold)
    lm_rel = LM_results[:lm_threshold]
    lm_non = LM_results[len(LM_results) - lm_threshold:]
    # print LM_results

    # Perform VSM query
    vsm_results = map(lambda x: x[0], vsm_query(query.strip(), term_dict, postings))
    vsm_threshold = int(len(vsm_results) * pseudo_relevance_threshold)
    vsm_rel = vsm_results[:vsm_threshold]
    vsm_non = vsm_results[len(vsm_results) - vsm_threshold:]
...
def combine_vectors(query_vector, relevant_vector, non_relevant_vector):
    ...
    relevant_vector_weight = 0.6
    non_relevant_vector_weight = -0.4
